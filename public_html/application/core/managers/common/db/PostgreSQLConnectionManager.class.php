...
class PostgreSQLConnectionManager {
	...
	public static function getInstance($pathConfig=null) {
		if (! isset ( PostgreSQLConnectionManager::$instance )) {
			PostgreSQLConnectionManager::$instance = new PostgreSQLConnectionManager ($pathConfig);
		}
		return PostgreSQLConnectionManager::$instance;
	}

	function __construct($pathConfig = null) {
		...
		if(is_null($pathConfig)){
			...
			$conf = parse_ini_file ( $pathConfig );
	...
	private function connect() {
		...
		try {
			...
			$connection_string .= " options='--client_encoding=UTF8'";
	...
	public function select($query, $objectid = "") {
		...
		try {
			$queryRS = pg_query ( $this->link, utf8_encode($query) );
