...
class LoginActionManager extends ActionManager {
	...
	protected function login() {
		...
		try{
			...
			$user = $userManager->getUserByLogin($_REQUEST['login_user']);
			$usertype = $userManager->getUserTypeId($_REQUEST['login_user']);
			...
			//var_dump($usertype);
			//var_dump($_SESSION['loggedUser']->getUserTypeId());
			//die();
			$urlhome = $this->getUrl ();
	...
	protected function loginPassPort() {
		...
		try{
			...
			if (trim ( $_REQUEST ['login_password'] ) != trim ( $user [0]->getUserPassword () ) && $user [0]->getUsertypeid () != EnumUserType::CLAIM_ADR) {
				throw new LoginException ( 'Password incorrect' );
			}
	...
	protected function noLogin(){
	   $userManager = UserManager::getInstance();
	   $user = $userManager->getUserByLogin("public");
	   $_SESSION ['loggedUser'] = $user[0];
           $urlhome = $this->getUrl ();
           $actionResponse  = new RedirectActionResponse ( $urlhome );
           $_SESSION['logger']->debug ( __METHOD__ . " end" );
