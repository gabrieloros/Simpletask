<?php
/**
 * Class to implement the response action Ajax Confirmation Popup
 * @package 
 * @author Gabriel Guzman
 * @version 1.0
 * DATE OF CREATION: 15/03/2012
 * DATE OF LAST UPDATE:
 * PURPOSE: Class to implement the response action for Ajax Confirmation Popup
 */
...
require_once $_SERVER ['DOCUMENT_ROOT'] . '/../application/views/popup/AdvancedPopup.view.php';

class AjaxConfirmBox extends AjaxActionResponse {
	...
	protected function buildJson() {
		...
		$basic = new PopupConfirm( $this->message, $this->title );
		
		$scroll = new PopupScrollBars ( $basic );
		
		$popup = new PopupBorders ( $scroll );
		
		$this->response = array (AjaxResponseType::CONFIRMBOX, array ($popup->draw (), $this->formId ) );
