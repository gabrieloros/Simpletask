...
class UtilOracle {
	...
	public static function getModuleUrlmapping($siteId, $languageId, $moduleName) {
		...
		$query = 'SELECT url FROM
					' . Util::getTableName ( "corp.urlmapping" ) . ' mapping
			    INNER JOIN ' . Util::getTableName ( "corp.content" ) . ' content		
						ON content.pfwid = mapping.idcontent ' . Util::getTableEnvironmentFilter ( 'content' ) . '
					INNER JOIN ' . Util::getTableName ( "corp.module" ) . ' module
						ON content.idmodule = module.pfwid ' . Util::getTableEnvironmentFilter ( 'module' ) . '
				...
				WHERE 
					...
					idlanguage = ' . $languageId . ' AND
					mapping.idsite = ' . $siteId . ' AND 
					(content.idsite=' . $siteId . ' OR content_site.idsite=' . $siteId . ')
					 ' . Util::getTableEnvironmentFilter ( 'mapping' );
		self::$logger->debug ( __METHOD__ . ' end' );
		return $query;
	}
