		...
		if(count($parsedData) > 0){
			...
			foreach ($parsedData as $row){
				...
				if(isset($row['A']) && $row['A'] != null){
					...
					$list = $this->manager->getCausesBySubject(claimsConcepts::ALUMBRADO_PUBLICO_SUBJECT_ID);
					...
					if($list !=null){
						...
						foreach ($list as $cause){
							...
							//var_dump($cause->getName());
							if ($cause->getName() == $causeName){

								$claimObj->setCauseId($cause->getId());
					...
					{
						$claimObj->setCauseId(claimsConcepts::SIN_ESTADO_ID);
					}

					//if ($claimObj->setCauseId() == null){
					//	$claimObj->setCauseId(claimsConcepts::SIN_ESTADO_ID);
					//}
