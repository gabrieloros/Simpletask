...
function validateForm(formId){
	...
	//Input url validate
	$('#' + formId + ' .input-url-validate').each(function(){

		inputValue = trimString($(this).val());
		
		if(inputValue != ''){
			if(!/(http|https):\/\//.test(inputValue)){
				$(this).css('border','1px solid red');
				errorsCount ++;
			}
			else{
	...
	if(errorsCount > 0){
		...
		errors = s_message['error_url_format'];
